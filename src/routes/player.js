...
function mapVideo(v) {
  return {
    ...
    youtube_id:   v.media_type === 'youtube' ? (v.filename.includes('|') ? v.filename.split('|')[0] : v.filename) : null,
    youtube_list: v.media_type === 'youtube' && v.filename.includes('|') ? v.filename.split('|')[1] : (v.media_type === 'youtube' && v.mime_type === 'youtube_playlist' ? v.filename : null),
